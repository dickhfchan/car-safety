...
def generate_dict_from_sql(query, conn, keep_datetime=True):
    ...
    #for every row in sql data
        
        if not keep_datetime:
        #Only if wanna convert to epoch timestamp
            row = []
            for i, data in enumerate(each):
            #iterate over attributes in this row and perform time conversion
                if type(data) in [datetime.datetime, datetime.date] and data is not None:
                    if columns[i] == "dob":
                        #For date of birth, they will be converted to string format as it can be before 1970-1-1 
...
def clean_veh_trip_table(conn, veh_trip_id):
    ...
    query = '''SELECT seq_no, lat, lng, unix_timestamp(time) as time FROM veh_trip_detail WHERE veh_trip_id=%s'''%(veh_trip_id) 
    ...
    df = df.loc[(df["lat"] != 0.) & (df["lng"] != 0.), ["seq_no", "lat", "lng", "time"]].dropna().sort_values(by="seq_no")
...
def retrieve_table(query_args, conn, json_format=False):
    ...
    if query_args[0] in ["veh_trip_detail", "veh_trip", "veh_trip_by_driver"]:
        ...
        query = "SELECT a.fullname, a.company_id, a.lang, map, group_id, user_id FROM user_account a, company b where a.company_id = b.company_id and a.username = '%s' and password = '%s' and b.company_code = '%s'"%(query_args[1], query_args[2], query_args[3])

        print "Authentication"
        print query

    elif query_args[0] in ['avg_warning_vrm_by_co_type_date', 'avg_warning_drv_by_co_type_date']:
        print query_args
	if query_args[0] == 'avg_warning_vrm_by_co_type_date':
            query =     "SELECT  `a`.`avg_warn_id` AS `avg_warn_id`, `a`.`vrm_id` AS `vrm_id`, `a`.`total_score` AS `total_score` ,  `a`.`type` AS `type`, `a`.`start_date` AS `start_date`,`a`.`drv_distance` AS `drv_distance`, `a`.`drv_duration` AS `drv_duration`,`a`.`pcw` AS `pcw`, `a`.`fcw` AS `fcw`, `a`.`ufcw_h` AS `ufcw_h`,`a`.`ufcw_l` AS `ufcw_l`,`a`.`vb` AS `vb`,`a`.`hmw_h` AS `hmw_h`,`a`.`hmw_m` AS `hmw_m`,`a`.`hmw_l` AS `hmw_l`,`a`.`lldw` AS `lldw`,`a`.`rldw` AS `rldw`,`a`.`spw` AS `spw`,`a`.`aaw` AS `aaw`, `a`.`abw` AS `abw`,`a`.`atw` AS `atw`,`b`.`company_id` AS `company_id`,`b`.`vrm_mark_code` AS `vrm_mark_code`,`c`.`vrm_grp_id` AS `vrm_grp_id` FROM ((`avg_warning_vrm` `a` JOIN `veh_reg_mark` `b`) JOIN `veh_reg_mark_group_dtl` `c`) WHERE ((`a`.`vrm_id` = `b`.`vrm_id`) AND (`a`.`vrm_id` = `c`.`vrm_id`) AND (a.vrm_id = %s) AND a.type = '%s' and start_date >= '%s' and start_date <= '%s')"%(query_args[1], query_args[2], query_args[3], query_args[4])
        else :
            query =     "SELECT  `a`.`avg_warn_id` AS `avg_warn_id`, `a`.`driver_id` AS `driver_id`, `a`.`total_score` AS `total_score` , `a`.`type` AS `type`, `a`.`start_date` AS `start_date`, `a`.`drv_distance` AS `drv_distance`, `a`.`drv_duration` AS `drv_duration`,`a`.`pcw` AS `pcw`, `a`.`fcw` AS `fcw`, `a`.`ufcw_h` AS `ufcw_h`,`a`.`ufcw_l` AS `ufcw_l`,`a`.`vb` AS `vb`,`a`.`hmw_h` AS `hmw_h`,`a`.`hmw_m` AS `hmw_m`,`a`.`hmw_l` AS `hmw_l`,`a`.`lldw` AS `lldw`,`a`.`rldw` AS `rldw`,`a`.`spw` AS `spw`,`a`.`aaw` AS `aaw`, `a`.`abw` AS `abw`,`a`.`atw` AS `atw`,`b`.`company_id` AS `company_id`,`b`.`name` AS `name` FROM ((`avg_warning_drv` `a` JOIN `driver` `b`)) WHERE ((`a`.`driver_id` = `b`.`driver_id`) AND (a.driver_id = %s) AND a.type = '%s' and start_date >= '%s' and start_date <= '%s')"%(query_args[1], query_args[2], query_args[3], query_args[4])
...
class from_Baidu(object):
    ...
    def get_track(start_time, end_time, entity):
	    ...
	    print "return blank for no result from baidu api"
	    return []
	    #return [(point['0'], point['0'], point['1498034127'])]
...
class CronJobs(object):
    ...
    def _run_baidu_api_privately(cls, company_id, conn):
        print "Find all veh_trip_ids for the company: " + str(company_id) 
        all_trip_ids = generate_dict_from_sql('SELECT b.veh_trip_id FROM veh_reg_mark a, veh_trip b where a.company_id = %s and a.vrm_id=b.vrm_id and b.veh_trip_id > 20000 ORDER BY b.veh_trip_id DESC'%company_id, conn)
        all_trip_ids = [ int(each.values()[0]) for each in all_trip_ids]
	print all_trip_ids

        print "veh_trip alrdy in Baidu table"
        baidu_id = generate_dict_from_sql("Select DISTINCT(veh_trip_id) FROM ui_baidu_snap_gps order by veh_trip_id desc", conn)
	...
	print last_id
        ...
        print "all veh_ids MINUS (ids alrdy in baidu table) MINUS (last ids of each vrm_id)"
	...
	print trimmed_ids
	    ...
	    print "Add entity"
            ...
            print "Add points"
            result = from_Baidu.add_points(conn=conn, veh_trip_id=trip_id, delay_between_calls=5)
            if result  > 0 :
	        print "Add pont to Baidu result :" + str(result)		
		#return result

            #Baidu Api #3: Get the points from baidu
            print "Get the point from Baidu"
		...
		print "Insert Blank record"
                result = [(0, 0, 1498034127)]
 	 	print type(result) 
		cls.insert_into_baidu_db(conn, result, trip_id)
		print " Insert one 0 records for trip %s"%trip_id
		print result
	    else :
		print "result :" + result 
                print " Unable to insert empty list into db for %s"%trip_id

	print "End of the Cronjob for company:" + str(company_id) 
	return 
